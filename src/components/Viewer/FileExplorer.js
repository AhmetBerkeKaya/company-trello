...
const FileExplorer = ({ projectId, onSelectPlan, selectedPlanId, onUploadSuccess }) => {
    ...
    const handleDeleteFile = async (e, file) => {
        e.stopPropagation();
        if (isObserver) return; // Gözlemci silemez

        const typeName = file.is_folder ? 'klasörünü' : 'dosyasını';
        if (!window.confirm(`"${file.name}" ${typeName} silmek istediğinize emin misiniz?`)) return;

        try {
            await api.delete(`/files/${file.file_id}`);
            // Eğer Firebase'den de fiziksel olarak silmek istersen burada storagePath ile silebilirsin.
            // Şimdilik sadece veritabanından ve ekrandan kaldırıyoruz:
            fetchFiles();
        } catch (error) {
                                        ...
                                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            {!file.is_folder && (
                                                <button
                                                    ...
                                                    className="p-1 hover:bg-white rounded-full text-gray-400 hover:text-blue-600 transition-colors"
                                            ...
                                            {/* Silme Butonu (Gözlemcilere Kapalı) */}
                                            {!isObserver && (
                                                <button
                                                    onClick={(e) => handleDeleteFile(e, file)}
                                                    className="p-1 hover:bg-white rounded-full text-gray-400 hover:text-red-600 transition-colors"
                                                    title="Sil"
                                                >
                                                    🗑️
                                                </button>
