...
// YENİ: onDragStart ve onDragEnd proplarını ekledik
const InteractivePin = ({ task, containerRef, onUpdate, onDelete, onDragStart, onDragEnd }) => {
  ...
  const handleMouseDown = (e) => {
    ...
    e.preventDefault();
    setIsDragging(true);
    if (onDragStart) onDragStart(); // YENİ: Haritaya "Sürükleme başladı, tıklamaları kilitle" de.
  };

  const handleMouseUp = async (e) => {
    if (!isDragging) return;
    setIsDragging(false);
    if (onDragEnd) onDragEnd(); // YENİ: Haritaya "Bitti, ama kilidi hemen açma" de.
    e.stopPropagation();
  ...
  return (
    ...
    >
      <div className={`w-8 h-8 rounded-full border-2 border-white shadow-md flex items-center justify-center text-white font-bold cursor-grab active:cursor-grabbing ${getPinColor()}`}>
        !
      </div>
